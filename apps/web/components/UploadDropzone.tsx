...
export default function UploadDropzone() {
  ...
  return (
    <div className="glass p-8">
      <label
        className="flex cursor-pointer flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed border-[var(--border)] p-10 text-center transition hover:border-brand"
      ...
      {phase === "uploading" && <p className="mt-4 text-center">上传中…</p>}
      {phase === "processing" && (
      ...
      {phase === "done" && (
        <p className="mt-4 text-center text-green-500">处理完成！（M1 将展示匹配岗位）</p>
      )}
      {phase === "error" && <p className="mt-4 text-center text-red-500">{message}</p>}
    </div>
  );
}
